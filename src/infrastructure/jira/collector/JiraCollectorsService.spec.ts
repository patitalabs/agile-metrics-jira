...
import { PtsCollectorConfig } from '../../../domain/project-tracking-system/Types';
...
describe('JiraCollectorsService', () => {
  ...
  const jiraCollectorsService: JiraCollectorsService =
    new JiraCollectorsService(jiraService);

  const config = ({ workFlowType }): PtsCollectorConfig =>
    new PtsCollectorConfig({
      teamId: 68,
  ...
  it('should fetch jiraMetrics for sprint', async () => {
    const collectorConfig = config({ workFlowType: 'sprint' });

    const data = await jiraCollectorsService.fetch(collectorConfig);
    expect(data).toMatchSnapshot();
  });

  it('should fetch jiraMetrics for kanban', async () => {
    const collectorConfig = config({ workFlowType: 'kanban' });
